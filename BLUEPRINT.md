...
- Does: searches the ticketing system for duplicates first. Writes a user story and acceptance criteria, asking the human only for what it cannot infer. Creates the ticket using the repo's title grammar and description template. Prints the ticket ID.
- Produces: one ticket. Nothing on disk.
- Never: cuts a branch, creates a worktree, or writes code. Ticket only.
...
- Never: merges, pushes to the default branch, or works outside its worktree.

**`/delegate review --self`**

- Takes: nothing. Runs in a ticket worktree on the current branch.
- Does: syncs the branch with `main`, diffs against it, and reviews the diff cold, in a fresh context. Fixes what it finds, commits the fixes, and pushes them. Fetches every existing comment on the PR first, so it never re-raises something already discussed.
...
- Does: pulls the PR into a detached review worktree, reads the full diff and every existing comment, reviews it, and posts findings as inline comments plus an approve or request changes verdict.
...
- Never: pushes, merges, opens a PR, leaves its worktree, or adds scope the plan did not include. Dispatched only by `/delegate implement`.
...
- Never: merges, leaves its worktree, picks up another ticket, or manages the epic. Dispatched only by `/orchestrate-work`.
...
- Returns: what it found, what it fixed, and what it left as residual risk.
- Never: merges, or pushes anything beyond its own review fix commits. Dispatched only by `/orchestrate-work`.
